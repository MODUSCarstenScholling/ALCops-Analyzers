...
| Skip triggers | Platform-defined names, can't rename |
| Skip interface implementations | Name is dictated by the interface |
| Skip event subscriber params | Subscriber parameters must match publisher signature (AL0828); platform trigger params (`xRec`, `BelowxRec`, `RunTrigger`, etc.) can't be renamed |
| Skip API object controls | API page/query controls require camelCase per AA0102; the default PascalCase pattern would always conflict |
| Skip whitespace-only names | `value(0; " ")` is a common "empty" enum value pattern; not a naming issue |
...
| Strip `&` accelerator for Action/Control | `&` before a character in action/control/group names is a Windows UI keyboard accelerator prefix (Alt+key shortcut), inherited from classic NAV/C/SIDE. Not stripped for other targets so fields/variables still flag `&` via their disallow pattern. |
...
| LocalVariable vs GlobalVariable as distinct targets | Enables different conventions for local vs global variables (e.g. `_` prefix only for locals). `Variable` remains as pure fallback parent, never directly dispatched to. |
| Parameter inherits from LocalVariable | Parameter naming is closer to local variable conventions than global. Multi-level chain: Parameter → LocalVariable → Variable. |
| VarParameter as distinct target | `var` parameters are passed by reference; teams may want different conventions (e.g. no underscore prefix). Inherits from `Parameter` → `LocalVariable` → `Variable`. |
