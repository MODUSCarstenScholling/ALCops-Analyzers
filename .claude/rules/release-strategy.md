...
Step-by-step procedure (branch, beta dispatch, stable tag, local tag cleanup, merge-back, tag hygiene): `.claude/skills/release/references/procedure.md`, driven by the `/release` skill which confirms every outward-facing step separately.
...
Workflow triggers and jobs are defined in `.github/workflows/`. The non-obvious behaviour: `build-and-release.yml` has two layers of protection against accidental prerelease tag pushes (e.g., from `git push --tags` re-pushing deleted beta tags):
