...
| Do not treat `ThrowError` as a terminator (removed in #464) | `ThrowError` is not an AL built-in: its only SDK occurrence is `CodeGenFacts.DataErrorThrowErrorName` (a C# code-generation helper), and `ErrorInfo` has no `Throw`/`ThrowError` member. The former check was dead code; do not re-add it. |
