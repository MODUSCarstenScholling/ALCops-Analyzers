...
[DiagnosticAnalyzer]
public sealed class IsHandledParameterAssignment : DiagnosticAnalyzer
{
    public override ImmutableArray<DiagnosticDescriptor> SupportedDiagnostics { get; } =
        ImmutableArray.Create(
            DiagnosticDescriptors.IsHandledParameterAssignment);

    public override void Initialize(AnalysisContext context)
    {
        context.RegisterOperationAction(
            CheckIsHandledAssignments,
            EnumProvider.OperationKind.AssignmentStatement);

        context.RegisterOperationAction(
            CheckIsHandledInvocations,
    ...
    private void CheckIsHandledAssignments(OperationAnalysisContext ctx)
    {
        ...
        if (assignment.Target.Kind != EnumProvider.OperationKind.ParameterReferenceExpression)
    ...
    private static bool IsIsHandledEventSubscriberParameter(IParameterSymbol parameter)
    {
        if (parameter.ContainingSymbol is null)
            return false;

        // check for event subscriber method
        if (parameter.ContainingSymbol.Kind != EnumProvider.SymbolKind.Method)
            return false;
        ...
        if (!CheckIsHandledName(parameter.Name) || (parameter.ParameterType.NavTypeKind != EnumProvider.NavTypeKind.Boolean) || !parameter.IsVar)
            return false;

        return true;
    }

    private static bool CheckIsHandledName(string name)
    {
    ...
    private static bool HasPrecedingExitStatement(IOperation operation, IParameterSymbol parameter)
    {
        ...
        while (parent.Kind != EnumProvider.SyntaxKind.Block && parent.Kind != EnumProvider.SyntaxKind.None)
        {
    ...
    private static bool IsSelfGuardedOrAssignment(IOperation operation)
    {
        // checks for the pattern: IsHandled := IsHandled or SomeOtherCondition;

        if (operation.Kind != EnumProvider.OperationKind.BinaryOperatorExpression)
            return false;
        IBinaryOperatorExpression operatorExpression = (IBinaryOperatorExpression)operation;

        var kind = operatorExpression.BinaryOperationKind;
        if (kind != EnumProvider.BinaryOperationKind.BooleanOr &&
            kind != EnumProvider.BinaryOperationKind.BooleanConditionalOr)
            return false;

        // check left side of or statement
        if (operatorExpression.LeftOperand.Kind == EnumProvider.OperationKind.ParameterReferenceExpression)
        {
        ...
        if (operatorExpression.RightOperand.Kind == EnumProvider.OperationKind.ParameterReferenceExpression)
        {
