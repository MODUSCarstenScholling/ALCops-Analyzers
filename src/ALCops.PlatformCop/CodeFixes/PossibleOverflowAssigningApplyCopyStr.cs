...
[CodeFixProvider(nameof(PossibleOverflowAssigningApplyCopyStrCodeFixProvider))]
public sealed class PossibleOverflowAssigningApplyCopyStrCodeFixProvider : CodeFixProvider
{
    ...
    private class PossibleOverflowAssigningApplyCopyStrCodeAction : CodeAction.DocumentChangeAction
    {
        ...
        public PossibleOverflowAssigningApplyCopyStrCodeAction(string title,
    ...
    private static PossibleOverflowAssigningApplyCopyStrCodeAction CreateCodeAction(SyntaxNode node, Document document, bool generateFixAll)
    {
        return new PossibleOverflowAssigningApplyCopyStrCodeAction(
            PlatformCopAnalyzers.PossibleOverflowAssigningApplyCopyStrCodeAction,
            ct => ApplyCopyStr(document, node, ct),
            nameof(PossibleOverflowAssigningApplyCopyStrCodeAction),
            generateFixAll);
    }
    ...
    private static async Task<Document> ApplyCopyStr(Document document, SyntaxNode node, CancellationToken cancellationToken)
    {
        ...
        if (node.GetFirstParent(EnumProvider.SyntaxKind.AssignmentStatement) is AssignmentStatementSyntax assignmentStatement)
            return await HandleAssignmentStatement(document, sourceExpression, assignmentStatement, cancellationToken);

        if (node.GetFirstParent(EnumProvider.SyntaxKind.ExitStatement) is ExitStatementSyntax exitStatementSyntax)
            return await HandleExitStatement(document, sourceExpression, exitStatementSyntax, cancellationToken);

        if (node.GetFirstParent(EnumProvider.SyntaxKind.InvocationExpression) is InvocationExpressionSyntax invocationExpression)
        ...
        ExitStatementSyntax exitStatement, CancellationToken cancellationToken)
    {
        ...
        if (exitStatement.GetFirstParent(EnumProvider.SyntaxKind.MethodDeclaration) is not MethodDeclarationSyntax methodDeclaration ||
