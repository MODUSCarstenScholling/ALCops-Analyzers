...
[DiagnosticAnalyzer]
public sealed class OptionTypeShouldBeEnum : DiagnosticAnalyzer
{
    public override ImmutableArray<DiagnosticDescriptor> SupportedDiagnostics { get; } =
        ImmutableArray.Create(
            DiagnosticDescriptors.OptionTypeShouldBeEnum);

    public override void Initialize(AnalysisContext context)
    {
        context.RegisterSyntaxNodeAction(
            new Action<SyntaxNodeAnalysisContext>(this.AnalyzeSyntaxNodes),
            EnumProvider.SyntaxKind.OptionDataType);
    ...
    {
        if (ctx.IsObsolete() || ctx.Node is not OptionDataTypeSyntax optionDataType)
            return;
    ...
    private static bool IsParameterOrReturnValue(OptionDataTypeSyntax optionDataType)
    {
        ...
        return parent is not null && (parent.Kind == EnumProvider.SyntaxKind.Parameter || parent.Kind == EnumProvider.SyntaxKind.ReturnValue);
    }
    ...
    private void AnalyzeVariables(SymbolAnalysisContext ctx)
    {
        ...
        {
            case var _ when variable.Kind == EnumProvider.SymbolKind.LocalVariable:
            ...
            case var _ when variable.Kind == EnumProvider.SymbolKind.GlobalVariable:
    ...
    private static List<IVariableSymbol> GetGlobalVariablesFromApplicationObject(IApplicationObjectTypeSymbol applicationObjectTypeSymbol)
    {
        ...
        {
            if (member.Kind == EnumProvider.SymbolKind.GlobalVariable && member is IVariableSymbol globalVar)
            {
                variables.Add(globalVar);
            }
            else if (member.Kind == EnumProvider.SymbolKind.Method && member is IMethodSymbol method)
            {
